...
When a task also changes Python code, read [`guidelines.md`](guidelines.md) before making code changes and keep the
implementation aligned with it.
...
2. If the task includes covered Python code changes, read `guidelines.md` before editing code.
3. Identify the externally observable behavior or invariant being added, changed, or removed.
4. Decide whether the change belongs in an existing contract or requires a new contract with a distinct responsibility.
5. Write or revise the contract using explicit normative language.
6. State defaults, configurable behavior, allowed values, failure behavior, side effects, and security constraints where relevant.
7. Update related contracts in the same pass so terminology and rules stay aligned.
8. Remove or rewrite implementation detail that is not externally required.
