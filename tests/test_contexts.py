...
from qpg.sources import add_source, delete_source, rename_source
...
def test_rename_source_updates_context_targets() -> None:
    conn = sqlite3.connect(":memory:")
    conn.row_factory = sqlite3.Row
    ensure_schema(conn)
    try:
        ...
        add_context(conn, "qpg://work", "work source context")
        add_context(conn, "qpg://work/public.orders", "work object context")
        add_context(conn, "qpg://work#abc123", "work fragment context")

        rename_source(conn, "work", "prod")

        remaining = list_contexts(conn)
        assert [row.target_uri for row in remaining] == [
            "qpg://prod",
            "qpg://prod/public.orders",
            "qpg://prod#abc123",
        ]
    finally:
        conn.close()
