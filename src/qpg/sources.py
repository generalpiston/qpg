...
def rename_source(conn: sqlite3.Connection, old_name: str, new_name: str) -> SourceRecord:
    ...
    old_root = f"qpg://{old_name}"
    new_root = f"qpg://{new_name}"
    rows = conn.execute(
        """
        SELECT id, target_uri
        FROM contexts
        WHERE target_uri = ?
           OR target_uri LIKE ?
           OR target_uri LIKE ?
        """,
        (old_root, f"{old_root}/%", f"{old_root}#%"),
    ).fetchall()
    for row in rows:
        target_uri = row["target_uri"]
        assert isinstance(target_uri, str)
        conn.execute(
            "UPDATE contexts SET target_uri = ? WHERE id = ?",
            (new_root + target_uri[len(old_root):], row["id"]),
        )
    conn.commit()
